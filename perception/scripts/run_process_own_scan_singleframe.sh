#!/bin/bash
#SBATCH --job-name=sq-ownscan-sf
#SBATCH --account=3dv
#SBATCH --gpus=1
#SBATCH --time=00:15:00
#SBATCH --output=/work/courses/3dv/team15/project-3dv/logs/own_scan_sf_%j.out

. /etc/profile.d/modules.sh
module add cuda/12.9
source /work/courses/3dv/team15/superdec/.venv/bin/activate

export PYTHONPATH="/work/courses/3dv/team15/curobo-sq/perception:/work/courses/3dv/team15/curobo-sq/superdec:${PYTHONPATH:-}"
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True

cd /work/courses/3dv/team15/project-3dv

python3 /work/courses/3dv/team15/curobo-sq/perception/scripts/process_own_scan.py \
  --r3d /work/courses/3dv/team15/data/own_scan/2026-03-30--20-51-15.r3d \
  --single-frame 3900 \
  --checkpoint /work/courses/3dv/team15/checkpoints/superdec_tabletop/superdec_tabletop_finetune_v3/epoch_300.pt \
  --xy-radius 3.0 \
  --min-height 0.01 \
  --max-height 0.35 \
  --min-seg-pts 80 \
  --exist-threshold 0.3 \
  --verbose \
  --output outputs/own_scan_singleframe2/
